...
spe_pseudo$domain <- factor(spe_pseudo$domain)
spe_pseudo <- spe_pseudo[, spe_pseudo$ncells >= 50]
colData(spe_pseudo)<-colData(spe_pseudo)[,c(21,22,24,31:33,159:162)]
colData(spe_pseudo)$broad.domain<-factor(
    ifelse(spe_pseudo$domain %in% levels(spe_pseudo$domain)[1:7],'Neuron',
           ifelse(spe_pseudo$domain %in% levels(spe_pseudo$domain)[8:11],'Neuropil',
                  ifelse(spe_pseudo$domain %in% levels(spe_pseudo$domain)[12:14],'WM',
                         'Vasc/CSF'))))
...
#Thal
spe_pseudo$thal<-ifelse(spe_pseudo$sample_id=='V11A20-297_B1' & spe_pseudo$domain %in% c('SUB','SUB.RHP'),T,F)
spe_pseudo<-spe_pseudo[,spe_pseudo$thal==F]
#Amy
spe_pseudo$amy<-ifelse(spe_pseudo$sample_id %in% c('V10B01-085_C1','V10B01-086_B1') & spe_pseudo$domain %in% c('RHP'),T,F)
spe_pseudo$amy<-ifelse(spe_pseudo$sample_id =='V10B01-085_A1',T,spe_pseudo$amy)
...
plotski<-jaffelab::getPcaVars(pca)[seq_len(50)]
# [1] 18.200  5.890  3.860  3.140  3.000  2.430  2.080  1.650  1.220  1.100  1.040  0.891  0.851  0.803
...
plotPCA(spe_pseudo, colour_by = "domain", ncomponents = 5, point_size = 1, point_alpha=1,label_format = c("%s %02i", " (%i%%)"),
        percentVar = metadata(spe_pseudo)$PCA_var_explained)+scale_color_manual(values=spatial.palette)+labs(color='domain')
...
# Add the ellipses
pdf(file=here::here('plots','figures','figure_2','pca_plot_final.pdf'),h=6,w=8)
...
levels(spe_pseudo$broad.domain)[3]<-'Vasc_CSF'
