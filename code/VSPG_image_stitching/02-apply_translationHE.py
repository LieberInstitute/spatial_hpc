...
sample_info_path = here('processed-data', '10-image_stitching', 'imageJ', 'transformationsHE', 'transformationsHE.csv')
combined_dir = Path(here('processed-data', '10-image_stitching', 'imageJ', f'combined_{this_donor}'))
...
#m_per_px = 3.610264905131402e-07
...
def merge_image_fullres(sample_info, theta, trans_img, max0, max1):
    ...
    #   image
    combined_img = np.full((max0 + 1, max1 + 687, 3, sample_info.shape[0]), 255, dtype = np.uint8)
    trans_img[4] = trans_img[4]+1000
    for i in range(sample_info.shape[0]):
    	...
    	img = np.array(img_pil.rotate(theta_deg, expand = True, fillcolor=BACKGROUND_COLOR), dtype = np.uint8)
    	#   Use separate channels for each image
    	combined_img[trans_img[i, 0]: trans_img[i, 0] + img.shape[0], trans_img[i, 1]: trans_img[i, 1] + img.shape[1] ,:, i] = img
...
tissue_positions_r['new_column'] = tissue_positions_r.index.str.split('-1_').str[1]
mask = tissue_positions_r['new_column'] == 'V11L05-335_D1'

# Update "pxl_row_in_fullres" and "pxl_col_in_fullres" of matching rows
tissue_positions_r.loc[mask, 'pxl_row_in_fullres'] += 1000
tissue_positions_r.loc[mask, 'pxl_col_in_fullres'] += 1000
tissue_positions_r.drop(columns=['new_column'], inplace=True)
