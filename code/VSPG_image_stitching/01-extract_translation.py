...
#sample_info['xml_path'] = sample_info['Slide #'].apply(lambda x: 'processed-data/VSPG_image_stitching/335.xml' if x.endswith('335') else 'processed-data/VSPG_image_stitching/332.xml')
...
for imagej_xml_path in sample_info['xml_path'].dropna().unique():
    ...
    this_sample_info = sample_info.loc[
        ['V12D07-' + slide_nums[i] for i in range(len(slide_nums))]
    ]
