...
midfile = read.csv(here("processed-data","VSPG_image_stitching", "broad2broad_deconvoVSPG.csv"), row.names =1)
split_midfile = split(midfile, midfile$tool) 
CART = split_midfile[[1]][c(1,3:8)]
colnames(CART)[3:7] <- paste('CART', colnames(CART)[3:7], sep = "_")
mid2broad_cell2location = split_midfile[[2]][c(1,3:8)]
...
mid2broad_RCTD = split_midfile[[3]][c(1,3:8)]
...
mid2broad_tangram = split_midfile[[4]][c(1,3:8)]
...
finefile = read.csv(here("processed-data","VSPG_image_stitching", "layer2broad_deconvoVSPG.csv"), row.names =1)
split_finefile = split(finefile, finefile$tool) 

fine2broad_cell2location = split_finefile[[2]][c(1,3:8)]
...
fine2broad_RCTD = split_finefile[[3]][c(1,3:8)]
...
fine2broad_tangram = split_finefile[[4]][c(1,3:8)]
...
df = cbind(CART,mid2broad_cell2location[,2:7], mid2broad_RCTD[,2:7], mid2broad_tangram[,2:7], fine2broad_cell2location[,2:7], fine2broad_RCTD[,2:7], fine2broad_tangram[,2:7])
write.csv(df, file = here("processed-data","VSPG_image_stitching", "deconvoVSPG.csv"))