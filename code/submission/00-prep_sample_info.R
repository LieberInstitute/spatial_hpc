...
he_sample_info_path = here(
    "code", "submission", "spe_sample_info.csv"
)
...
he_info = read_tsv(
  ...
  mutate(
    ...
    spaceranger_json = here(
      "processed-data", "01_spaceranger", "spaceranger-all", sample_id, "outs", "spatial",
...
if_info = read_tsv(
    ...
    mutate(
        ...
        stain = "IF",
        spaceranger_json = here(
            "processed-data", "01_spaceranger", "spaceranger_VSPG", sample_id, "outs", "spatial",
            "scalefactors_json.json"
        )
    ) |>
    rename(image_file = X4, alignment_file = X5) |>
