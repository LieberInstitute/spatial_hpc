...
#   We use some modified tangram code to circumvent the need for image segmentation, instead just relying on per-spot cell counts
os.chdir(pyhere.here("code", "spot_deconvo", "tangram"))
...
plot_dir = pyhere.here("plots", "spot_deconvo", "tangram", "IF", "2ndRun_newClass", cell_group)
processed_dir = pyhere.here("processed-data", "spot_deconvo", "tangram", "IF", "2ndRun_newClass", cell_group)
...
tg.plot_cell_annotation_sc(ad_sp, annotation_list, perc=0.02, scale_factor = SCALE_FACTOR)
f = plt.gcf()
f.savefig(
    os.path.join(
