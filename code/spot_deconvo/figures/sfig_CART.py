...
plot_dir = pyhere.here('plots','spot_deconvo', 'figures')
...
def plot_roi(img, props, indices, vmax: int = 128, pad: int = 25):
    ...
    for idx in indices:
        ...
        j += 1
        for i in range(1, 6):
            axs[j].yaxis.set_tick_params(labelleft=False)
            axs[j].set_yticks([])        
            a = axs[j].imshow(
                imgs[
                    i-1,
                    max(0, bbox[0] - pad) : min(imgs.shape[1], bbox[2] + pad),
                    max(0, bbox[1] - pad) : min(imgs.shape[2], bbox[3] + pad),
                ],
                vmax=vmax,
                aspect="equal", 
            )   
