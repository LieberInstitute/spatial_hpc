...
png(here("plots","spot_deconvo","figures","fig_benchmark", "sfig_layer_RMSEvsCorr.png"), width = 1200, height = 400, units = "px") 
p = ggplot(metrics_df, aes(x = corr, y = rmse, color = variable, shape = sample))+geom_point(size = 5)+ theme_bw() +
  facet_wrap(~tool, nrow=1)+ scale_color_manual(values = c("green", "darkorange", "yellow", "magenta"))+
