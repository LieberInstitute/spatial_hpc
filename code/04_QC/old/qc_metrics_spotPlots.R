...
pdf(here("plots", "04_QC", "QC_discard_lowFeaturesCount_by_brain.pdf"), width = 21, height = 10)
...
pdf(here("plots", "04_QC", "QC_discard_lowFeaturesCount_by_brain_grid.pdf"), width = 21, height = 20)
...
pdf(here("plots", "04_QC", "QC_discard_lowFeaturesCount_by_capture_area.pdf"), width = 21, height = 10)
samples <- unique(colData(spe)[, c("sample_id", "brnum")])
rownames(samples) <- NULL
...
pdf(here("plots", "04_QC", "QC_discard_lowFeaturesCount_by_capture_area_grid.pdf"), width = 21, height = 20)
brains = unique(spe$brnum)
for (i in seq_along(brains)){
    speb <- spe[, which(spe$brnum == brains[i])]
    samples <- unique(speb$sample_id)
...
pdf(here("plots", "04_QC", "QC_discard_highMito_by_brain.pdf"), width = 21, height = 20)
for (i in seq_along(brains)){
    speb <- spe[, which(spe$brnum == brains[i])]
    samples <- unique(speb$sample_id)
    samples
...
pdf(here("plots", "04_QC", "QC_discard_highMito_by_capture_area.pdf"), width = 21, height = 20)
for (i in seq_along(brains)){
    speb <- spe[, which(spe$brnum == brains[i])]
    samples <- unique(speb$sample_id)
    samples
