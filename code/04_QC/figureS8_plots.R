...
ggplot(df, aes(x = Sample, y = count, fill = 'firebrick2')) +
...
pdf(file=here::here('plots','figures','supp_figures','clustering1_UMAP_SYT1.pdf'),h=3.5,w=3.5)

df<-as.data.frame(table(sce$Sample))
colnames(df)<-c('Sample','sort','nuclei')
df<-df[!df$nuclei==0,]
