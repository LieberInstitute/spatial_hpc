...
for (j in seq_along(brains)) {
  ...
  p[[j]]<-plotVisium(speb, spots=T, image=F, highlight="binary_label", annotate="nmf65",
    ...
    scale_fill_gradient(low="white", high="black", limits=c(0, max(spe$nmf65)), guide="none")+
...
lplot = ggplot(cbind.data.frame("y"=seq(binary.palette), "labels"=names(binary.palette),
                                "nmf65"=seq(from=0, to=max(spe$nmf65), length.out=length(binary.palette))),
               aes(x=1, y=y, color=labels, fill=nmf65))+
...
pdf(file = "plots/revision/all-brain_spotplot_binary-nmf65.pdf",
    width=9, height=12)
