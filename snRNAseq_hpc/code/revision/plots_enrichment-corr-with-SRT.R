...
spe_stats = stats$enrichment[,grep("t_stat",colnames(stats$enrichment))]
colnames(spe_stats)<-gsub(pattern='t_stat_',
                          replacement='',
                          colnames(spe_stats))
...
### needed to fix names when we used annotated clusters (with thal and amy)
...
new.names = intersect(rownames(spe_stats), rownames(sn_stats))
length(new.names)
#14951
